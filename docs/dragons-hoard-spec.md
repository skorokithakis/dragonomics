...
* **Conversation.** A parley of N participants runs N rounds. Each round visits every participant once in random order; each may speak one message or pass. A round where at most one participant speaks ends the parley early. When the rounds end, the conversation is over until the next window.
