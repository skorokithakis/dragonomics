...
* A parley of N participants runs N rounds; each round visits every participant once in random order; speak one message or pass. A full round of silence ends the parley early. Conversation length scaling with group size is deliberate: a pair gets a tight exchange, a five-thief cabal gets a real meeting that produces a long transcript any member can betray. Big conspiracies are naturally expensive and leaky.
